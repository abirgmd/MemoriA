...
export class ProfileAccompagnantComponent {
  ...
  onSubmit(): void {
    ...
    // ✅ Payload compatible avec AccompagnantSignupData
    const payload: AccompagnantSignupData = {
      lienPatient: rawValue.lienPatient,
      dateNaissance: rawValue.dateNaissance,
      frequenceAccompagnement: rawValue.frequenceAccompagnement,
      adresse: rawValue.adresse || undefined,
      codePostal: rawValue.codePostal || undefined,
      ville: rawValue.ville || undefined,
      telephoneSecours: rawValue.telephoneSecours || undefined,
      situationPro: rawValue.situationPro === '' 
        ? undefined 
        : rawValue.situationPro
    };

    this.accompagnantService.saveProfile(this.currentUser.id, payload).subscribe({
      next: () => {
        this.authService.markProfileCompleted();
        this.successMessage = 'Profil accompagnant enregistré.';
      ...
      error: (err: any) => {
        ...
        const errorMsg =
          err?.error?.message ||
          err?.statusText ||
          'Impossible de sauvegarder le profil accompagnant.';
  ...
  private loadExistingProfile(): void {
    ...
    this.accompagnantService.getProfile(this.currentUser.id)
