    ...
    this.authService.login(this.loginForm.getRawValue()).subscribe({
      next: (user) => {
        ...
        if (user.role.toUpperCase() === 'SOIGNANT') {
          this.router.navigate(['/doctor_planning']);
          return;
        }
        if (user.role.toUpperCase() === 'PATIENT') {
          if (!user.profileCompleted) {
            ...
            this.router.navigate(['/patient_planning']);
          }
          return;
        }
        if (user.role.toUpperCase() === 'ACCOMPAGNANT') {
          if (!user.profileCompleted) {
            this.router.navigate(['/profile/accompagnant']);
          } else {
            this.router.navigate(['/caregiver_planning']);
          }
          return;
        }
